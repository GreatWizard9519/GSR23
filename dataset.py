...
def process_dataset(name, epsilon, subgraph_sampling=None, ratio=None, adj_init='cos_sim', adj_init_random_p=0.5, alpha1=0.2, alpha2=0.4, sim_k=5):
    if name == 'cora':
    ...
    if subgraph_sampling == 'metis' and not name == 'fraudyelp':
        if name == 'pubmed':
            part_no = 1
        elif name == 'reddit':
            part_no = 4
    ...
    if adj_init == 'ori':
        ...
        KNN = sim_k
        init_graph_adj = np.zeros((n_nodes, n_nodes))
        # if not os.path.exists(f'save/{name}_cosine_sim.npy'):
    ...
    np.save('target_sv.npy', target_sv)
    np.save('target_u.npy', target_u)
